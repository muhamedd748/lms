...
def format_remaining_time(minutes: int) -> str:
    minutes = abs(minutes)
    if minutes < 60:
        return f"{minutes} minute{'s' if minutes_past != 1 else ''}"
    hours = minutes // 60
    mins = minutes % 60
    if mins == 0:
...
# ================== FIXED FETCH DATA (Better SGCaptcha Detection) ==================
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            for attempt in range(6):
                if attempt > 0:
                    await asyncio.sleep(random.uniform(2.0, 4.0))  # Slightly longer delay to look more human
                ...
                # IMPROVED: Better detection for SiteGround SGCaptcha
                lower_text = raw_text.lower()
                if (lower_text.startswith(('<html', '<!doctype'))) or \
                   ('sgcaptcha' in lower_text) or \
                   ('robot challenge' in lower_text) or \
                   ('verify you are human' in lower_text) or \
                   ('/.well-known/sgcaptcha' in lower_text):
                    logger.error("🚫 Server returned SGCaptcha / protection page")
                    logger.error(f"Preview: {raw_text[:500]}")
                    continue
...
# ================== BUTTON HANDLER ==================
# (Your original button_handler remains unchanged - only fetch_data was fixed)
