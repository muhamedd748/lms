    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            for attempt in range(6):
                ...
                logger.info(f"📡 Attempt {attempt+1}/6 | Status: {response.status_code} | Encoding: {response.headers.get('content-encoding')}")
                ...
                if encoding == "gzip":
                    ...
                    try:
                        ...
                        logger.info("✅ Brotli decompressed")
                    except Exception as e:
                        logger.error(f"Brotli error: {e}")
                ...
                try:
                    ...
                    logger.warning("Used replace decoding")

                if len(text) < 50 or text.strip().startswith("<html"):
                    logger.error("Received HTML or invalid data")
